...
const handleRemoveCourse = async () => {
  ...
  <div className='space-x-2 space-y-2 '>
  { !isPublished ? <button className='bg-green-100 text-green-600 px-4 py-2 rounded-md border-1 ' onClick={()=>setIsPublished(prev=>!prev)}>Clik to Publish</button>:   <button className='bg-red-100 text-red-600 px-4 py-2 rounded-md border-1 '  onClick={()=>setIsPublished(prev=>!prev)}>Clik to UnPublish</button>}
    ...
    <div className='flex flex-col sm:flex-row sm:space-x-4 space-y-4 sm:space-y-0'>
      ...
      <div className='flex-1'>
       ...
       <label htmlFor="price" className='block text-sm font-medium text-gray-700 mb-1'>Course Price (NPR)</label>
        <input type="number" name='' id='price' className='w-full border px-4 py-2 rounded-md' placeholder='Rs' onChange={(e)=>setPrice(e.target.value)} value={price} />
      </div>
