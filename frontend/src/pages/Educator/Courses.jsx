...
<button className='bg-[black] text-white px-4 py-2 rounded hover:bg-gray-500 ' onClick={()=>navigate("/createcourse")}>Create Courses</button>
   ...
   <div className='hidden md:block bg-white rounded-xl shadow p-4 overflow-x-auto '>
    <table className='min-w-full text-sm'>
      ...
      <tbody>
        ...
        <tr key={index} className='border-b hover:bg-gray-50 transition duration-200 '>
          ...
          { course?.price ? <td className='px-4 py-3'>Rs{course?.price}</td>:
    ...
    <p className='text-center text-sm text-gray-400 mt-6'>A list of your recent course.</p>


   </div>
 
   {/* for small Screen table   */}
   <div className='md:hidden soace-y-4'>
     {creatorCourseData?.map((course , index)=>(
    <div key={index} className='bg-white rounded-lg shadow p-4 flex flex-col gap-3'>
      ...
      { course?.price ?<p className='text-gray-600 text-xs mt-1'>Rs{course?.price}</p>:<p className='text-gray-600 text-xs mt-1'>Rs NA</p>}
        </div>
        < FaEdit className='text-gray-600 hover:text-blue-600 cursor-pointer '  onClick={()=>navigate(`/editcourse/${course?._id}`)}/> 
      </div>
